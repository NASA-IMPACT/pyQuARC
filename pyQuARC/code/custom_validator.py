...
class CustomValidator(BaseValidator):
    ...
    @staticmethod
    @if_arg
    def ends_at_present_flag_logic_check(ends_at_present_flag, ending_date_time, collection_state):
        value = ends_at_present_flag.lower()
        collection_state = collection_state.upper()

        valid = (value == "true" and not (ending_date_time) or collection_state == "ACTIVE") or (
            value == "false" and ending_date_time or collection_state == "COMPLETE"
        )
    ...
    def bounding_coordinate_logic_check(coordinates_dictionary):
        ...
        values = {
            coordinate: int(coordinates_dictionary.get(coordinate, 0)) for coordinate in coordinates
        }
