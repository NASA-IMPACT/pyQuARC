...
class TestCustomChecker:
    ...
    def test_get_path_value(self):
        ...
        dummy_dif_metadata = {
            ...
            "MetadataDates": [
                {"Type": "CREATE", "Date": "2021-09-15T15:54:00.000Z"},
                {"Type": "UPDATE", "Date": "2021-09-30T15:54:00.000Z"},
            ],
            "DOI": {"Authority": "https://doi.org", "DOI": "10.5067/MODIS/MOD13Q1.061"},
            "SpatialExtent": {
        ...
        assert CustomChecker._get_path_value(
            dummy_dif_metadata, "CollectionCitations/Creator"
        ) == ["Kamel Didan"]

        assert CustomChecker._get_path_value(
            dummy_dif_metadata, "CollectionCitations/OnlineResource/Name"
        ) == ["DOI Landing Page"]

        assert CustomChecker._get_path_value(
            dummy_dif_metadata, "MetadataDates/Date?Type=UPDATE"
        ) == ["2021-09-30T15:54:00.000Z"]

        assert CustomChecker._get_path_value(
            dummy_dif_metadata, "MetadataDates/Date?Type=CREATE"
        ) == ["2021-09-15T15:54:00.000Z"]

        assert CustomChecker._get_path_value(dummy_dif_metadata, "DOI/DOI") == [
        ...
        assert CustomChecker._get_path_value(
            dummy_dif_metadata, "SpatialExtent/GranuleSpatialRepresentation"
        ) == ["GEODETIC"]
